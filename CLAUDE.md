...
| **Site web + RS485 KO après reboot, mais SSH OK** (service en `activating (start)`, PID qui change toutes les ~5 min) | redb fait une **récupération** après arrêt non gracieux (reboot brutal) ; sur grosse base elle dépassait l'ancien `TimeoutStartSec=300` → systemd tuait/relançait en **boucle infinie** → port 8080 jamais ouvert **et** RS485 jamais démarré (le code bloque à l'ouverture redb, bien avant `/dev/ttyUSB0`). **Corrigé 2026-07-09** : `TimeoutStartSec=infinity` (`contrib/daly-bms.service`) → systemd attend la fin de la recovery, la taille du fichier n'est plus un facteur de panne. Diag : `systemctl status daly-bms` (state `Dsl`, CPU qui grimpe, `read_bytes` >> taille fichier = recovery en cours). Remise en route immédiate sans attendre : `sudo systemctl stop daly-bms && sudo mv /mnt/nvme/daly-bms/metrics.redb /mnt/nvme/daly-bms/metrics.redb.bloat.<ts> && sudo systemctl start daly-bms` (repart base vide ; historique conservé dans le fichier renommé → récupérable hors-ligne via `--compact-db`). |

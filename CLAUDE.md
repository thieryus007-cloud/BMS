...
| **Site web + RS485 KO après reboot, mais SSH OK** (service en `activating (start)`, PID qui change toutes les ~5 min) | redb fait une **récupération** après arrêt non gracieux (reboot brutal) ; sur grosse base elle dépassait l'ancien `TimeoutStartSec=300` → systemd tuait/relançait en **boucle infinie** → port 8080 jamais ouvert **et** RS485 jamais démarré (le code bloque à l'ouverture redb, bien avant `/dev/ttyUSB0`). **Corrigé 2026-07-09, 2 volets** : **(1)** `TimeoutStartSec=infinity` (`contrib/daly-bms.service`) → systemd attend la fin de la recovery, la taille du fichier n'est plus un facteur de panne. **(2)** L'**ouverture redb se fait en tâche de fond** (`main.rs`, `state.set_metrics_store()`) → le site HTTP + RS485 sont dispo **immédiatement** après le boot ; la persistance des métriques + les lectures Grafana reprennent dès la fin de la recovery (pendant l'intervalle, backend `None` → écritures ignorées, lectures « backend not ready »). Diag : `systemctl status daly-bms` (désormais `active` vite ; log `metrics-store : ouverture en tâche de fond…` puis `metrics-store ouvert … backend disponible` avec `elapsed_s`). Remise en route immédiate sans attendre la recovery : `sudo systemctl stop daly-bms && sudo mv /mnt/nvme/daly-bms/metrics.redb /mnt/nvme/daly-bms/metrics.redb.bloat.<ts> && sudo systemctl start daly-bms` (repart base vide ; historique conservé dans le fichier renommé → récupérable hors-ligne via `--compact-db`). **Ne protège PAS d'une corruption avérée** (quarantaine → base vide) : d'où les sauvegardes quotidiennes `daly-bms-backup.timer` (#3). |

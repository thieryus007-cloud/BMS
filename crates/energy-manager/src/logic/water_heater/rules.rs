...
impl IrradianceRuleEngine {
    ...
    pub fn validate(&mut self, raw: f64) -> anyhow::Result<bool> {
        let mut facts = Facts::new(); // ✅ mut
        facts.set("IR.raw", Value::Number(raw)); // ✅ pas d'espace
        facts.set("IR.valid", Value::Boolean(false)); // ✅ pas d'espace

        self.engine
            .execute(&mut facts) // ✅ &mut
            .context("Irradiance rule engine evaluation failed")?;

        Ok(facts
            .get("IR.valid") // ✅ pas d'espace
            .and_then(|v| match v {

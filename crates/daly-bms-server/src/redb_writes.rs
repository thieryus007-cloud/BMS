...
/// n'est construit QUE si le rate-limiter autorise l'écriture. Avec l'ancien
/// passage par valeur, ~80 % des `Sample` construits partaient à la poubelle
/// (snapshots à ~1 Hz, écriture 1×/5 s par série) — churn d'allocations
/// continu sur le chemin passif (investigation RSS,
/// docs/diagnostic-depannage.md §17).
fn push(
    writer: &Writer,
    rl: &RateLimiter,
    interval: Duration,
...
pub fn write_bms(writer: &Writer, rl: &RateLimiter, snap: &BmsSnapshot) {
    ...
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_voltage:{}", bms_id),
         || Sample::new("bms_voltage", ts, snap.dc.voltage as f64)
            .with_label("bms_id", bms_id.clone()));

    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_current:{}", bms_id),
         || Sample::new("bms_current", ts, snap.dc.current as f64)
            .with_label("bms_id", bms_id.clone()));

    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_soc:{}", bms_id),
         || Sample::new("bms_soc", ts, snap.soc as f64)
            .with_label("bms_id", bms_id.clone()));

    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_power:{}", bms_id),
         || Sample::new("bms_power", ts, snap.dc.power as f64)
            .with_label("bms_id", bms_id.clone()));

    push(writer, rl, TEMP_WRITE_INTERVAL, &format!("bms_temp_max:{}", bms_id),
         || Sample::new("bms_temp_max", ts, snap.system.max_cell_temperature as f64)
            .with_label("bms_id", bms_id.clone()));

    push(writer, rl, TEMP_WRITE_INTERVAL, &format!("bms_temp_min:{}", bms_id),
         || Sample::new("bms_temp_min", ts, snap.system.min_cell_temperature as f64)
            .with_label("bms_id", bms_id.clone()));

    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_cell_delta_mv:{}", bms_id),
    ...
    push(writer, rl, ENERGY_WRITE_INTERVAL, &format!("bms_capacity_ah:{}", bms_id),
    ...
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_charge_mos:{}", bms_id),
         || Sample::new("bms_charge_mos", ts, snap.io.allow_to_charge as f64)
            .with_label("bms_id", bms_id.clone()));

    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_discharge_mos:{}", bms_id),
    ...
    for (key, &voltage) in snap.voltages.iter() {
        ...
        push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_cell_voltage:{}:{}", bms_id, cell_idx),
    ...
    for (key, &active) in snap.balances.iter() {
        let cell_idx = key.trim_start_matches("Cell");
        push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_cell_balancing:{}:{}", bms_id, cell_idx),
    ...
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_soh:{}", bms_id),
         || Sample::new("bms_soh", ts, snap.soh as f64)
            .with_label("bms_id", bms_id.clone()));
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_time_to_go_secs:{}", bms_id),
         || Sample::new("bms_time_to_go_secs", ts, snap.time_to_go as f64)
            .with_label("bms_id", bms_id.clone()));

    // ── États bool/flag ──────────────────────────────────────────────────────
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_balancing_active:{}", bms_id),
         || Sample::new("bms_balancing_active", ts, snap.balancing as f64)
            .with_label("bms_id", bms_id.clone()));
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_system_switch:{}", bms_id),
         || Sample::new("bms_system_switch", ts, snap.system_switch as f64)
            .with_label("bms_id", bms_id.clone()));
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_heating_active:{}", bms_id),
         || Sample::new("bms_heating_active", ts, snap.heating as f64)
            .with_label("bms_id", bms_id.clone()));

    // ── Tensions / température DC ───────────────────────────────────────────
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_min_cell_voltage:{}", bms_id),
         || Sample::new("bms_min_cell_voltage", ts, snap.system.min_cell_voltage as f64)
            .with_label("bms_id", bms_id.clone()));
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_max_cell_voltage:{}", bms_id),
         || Sample::new("bms_max_cell_voltage", ts, snap.system.max_cell_voltage as f64)
            .with_label("bms_id", bms_id.clone()));
    push(writer, rl, TEMP_WRITE_INTERVAL, &format!("bms_mos_temp_c:{}", bms_id),
         || Sample::new("bms_mos_temp_c", ts, snap.system.mos_temperature as f64)
            .with_label("bms_id", bms_id.clone()));

    // ── Capacités (Ah) ───────────────────────────────────────────────────────
    push(writer, rl, ENERGY_WRITE_INTERVAL, &format!("bms_consumed_ah:{}", bms_id),
         || Sample::new("bms_consumed_ah", ts, snap.consumed_amphours as f64)
            .with_label("bms_id", bms_id.clone()));
    push(writer, rl, ENERGY_WRITE_INTERVAL, &format!("bms_capacity_remaining_ah:{}", bms_id),
         || Sample::new("bms_capacity_remaining_ah", ts, snap.capacity as f64)
            .with_label("bms_id", bms_id.clone()));
    push(writer, rl, ENERGY_WRITE_INTERVAL, &format!("bms_reported_capacity_ah:{}", bms_id),
         || Sample::new("bms_reported_capacity_ah", ts, snap.bms_reported_capacity_ah as f64)
            .with_label("bms_id", bms_id.clone()));

    // ── Modules ──────────────────────────────────────────────────────────────
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_modules_online:{}", bms_id),
         || Sample::new("bms_modules_online", ts, snap.system.nr_of_modules_online as f64)
            .with_label("bms_id", bms_id.clone()));
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_modules_offline:{}", bms_id),
         || Sample::new("bms_modules_offline", ts, snap.system.nr_of_modules_offline as f64)
            .with_label("bms_id", bms_id.clone()));
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_modules_blocking_charge:{}", bms_id),
         || Sample::new("bms_modules_blocking_charge", ts, snap.system.nr_of_modules_blocking_charge as f64)
            .with_label("bms_id", bms_id.clone()));
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_modules_blocking_discharge:{}", bms_id),
    ...
    for (name, v) in alarms {
        push(writer, rl, MIN_WRITE_INTERVAL, &format!("{}:{}", name, bms_id),
             || Sample::new(name, ts, v as f64).with_label("bms_id", bms_id.clone()));
    }

    // ── Historique de vie ────────────────────────────────────────────────────
    push(writer, rl, ENERGY_WRITE_INTERVAL, &format!("bms_charge_cycles:{}", bms_id),
         || Sample::new("bms_charge_cycles", ts, snap.history.charge_cycles as f64)
            .with_label("bms_id", bms_id.clone()));
    push(writer, rl, ENERGY_WRITE_INTERVAL, &format!("bms_min_voltage_hist:{}", bms_id),
         || Sample::new("bms_min_voltage_hist", ts, snap.history.minimum_voltage as f64)
            .with_label("bms_id", bms_id.clone()));
    push(writer, rl, ENERGY_WRITE_INTERVAL, &format!("bms_max_voltage_hist:{}", bms_id),
         || Sample::new("bms_max_voltage_hist", ts, snap.history.maximum_voltage as f64)
            .with_label("bms_id", bms_id.clone()));
    push(writer, rl, ENERGY_WRITE_INTERVAL, &format!("bms_total_ah_drawn:{}", bms_id),
         || Sample::new("bms_total_ah_drawn", ts, snap.history.total_ah_drawn as f64)
            .with_label("bms_id", bms_id.clone()));

    // ── Limites de charge ───────────────────────────────────────────────────
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_charge_request:{}", bms_id),
         || Sample::new("bms_charge_request", ts, snap.info.charge_request as f64)
            .with_label("bms_id", bms_id.clone()));
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_max_charge_voltage:{}", bms_id),
         || Sample::new("bms_max_charge_voltage", ts, snap.info.max_charge_voltage as f64)
            .with_label("bms_id", bms_id.clone()));
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_max_charge_current:{}", bms_id),
         || Sample::new("bms_max_charge_current", ts, snap.info.max_charge_current as f64)
            .with_label("bms_id", bms_id.clone()));
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_max_discharge_current:{}", bms_id),
         || Sample::new("bms_max_discharge_current", ts, snap.info.max_discharge_current as f64)
            .with_label("bms_id", bms_id.clone()));
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_max_charge_cell_voltage:{}", bms_id),
         || Sample::new("bms_max_charge_cell_voltage", ts, snap.info.max_charge_cell_voltage as f64)
            .with_label("bms_id", bms_id.clone()));

    // ── IO permissions (autorisations BMS) ──────────────────────────────────
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_balance_mos:{}", bms_id),
         || Sample::new("bms_balance_mos", ts, snap.io.allow_to_balance as f64)
            .with_label("bms_id", bms_id.clone()));
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_heat_mos:{}", bms_id),
         || Sample::new("bms_heat_mos", ts, snap.io.allow_to_heat as f64)
            .with_label("bms_id", bms_id.clone()));
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_external_relay:{}", bms_id),
         || Sample::new("bms_external_relay", ts, snap.io.external_relay as f64)
            .with_label("bms_id", bms_id.clone()));

    // ── Chauffage (consigne / consommation) ─────────────────────────────────
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_heating_current:{}", bms_id),
         || Sample::new("bms_heating_current", ts, snap.info.heating_current as f64)
            .with_label("bms_id", bms_id.clone()));
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("bms_heating_power:{:02x}", snap.address),
...
pub fn write_et112(writer: &Writer, rl: &RateLimiter, snap: &Et112Snapshot) {
    ...
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("et112_power_w:{}", addr),
         || Sample::new("et112_power_w", ts, snap.power_w as f64)
            .with_label("address", addr.clone()));

    push(writer, rl, MIN_WRITE_INTERVAL, &format!("et112_voltage_v:{}", addr),
         || Sample::new("et112_voltage_v", ts, snap.voltage_v as f64)
            .with_label("address", addr.clone()));

    push(writer, rl, MIN_WRITE_INTERVAL, &format!("et112_current_a:{}", addr),
         || Sample::new("et112_current_a", ts, snap.current_a as f64)
            .with_label("address", addr.clone()));

    push(writer, rl, MIN_WRITE_INTERVAL, &format!("et112_apparent_power_va:{}", addr),
         || Sample::new("et112_apparent_power_va", ts, snap.apparent_power_va as f64)
            .with_label("address", addr.clone()));

    push(writer, rl, MIN_WRITE_INTERVAL, &format!("et112_power_factor:{}", addr),
         || Sample::new("et112_power_factor", ts, snap.power_factor as f64)
            .with_label("address", addr.clone()));

    push(writer, rl, MIN_WRITE_INTERVAL, &format!("et112_frequency_hz:{}", addr),
         || Sample::new("et112_frequency_hz", ts, snap.frequency_hz as f64)
            .with_label("address", addr.clone()));

    push(writer, rl, MIN_WRITE_INTERVAL, &format!("et112_reactive_power_var:{}", addr),
         || Sample::new("et112_reactive_power_var", ts, snap.reactive_power_var as f64)
            .with_label("address", addr.clone()));

    // Énergie cumulée en Wh (compteur monotone).
    push(writer, rl, ENERGY_WRITE_INTERVAL, &format!("et112_energy_import_wh:{}", addr),
         || Sample::new("et112_energy_import_wh", ts, snap.energy_import_kwh() as f64 * 1000.0)
            .with_label("address", addr.clone()));

    push(writer, rl, ENERGY_WRITE_INTERVAL, &format!("et112_energy_export_wh:0x{:02x}", snap.address),
...
pub fn write_irradiance(writer: &Writer, rl: &RateLimiter, snap: &IrradianceSnapshot) {
    let ts = now_ms();
    push(writer, rl, MIN_WRITE_INTERVAL, "irradiance_wm2",
...
pub fn write_venus_mppt(writer: &Writer, rl: &RateLimiter, mppt: &VenusMppt) {
    ...
    if let Some(state_code) = mppt.state.as_ref().and_then(|s| mppt_state_to_code(s)) {
        push(writer, rl, MIN_WRITE_INTERVAL, &format!("venus_mppt_state:{}", instance),
...
pub fn write_venus_smartshunt(writer: &Writer, rl: &RateLimiter, shunt: &VenusSmartShunt) {
    ...
    if let Some(v) = shunt.soc_percent {
        push(writer, rl, MIN_WRITE_INTERVAL, "venus_shunt_soc_percent",
             || Sample::new("venus_shunt_soc_percent", ts, v as f64));
    }
    if let Some(v) = shunt.voltage_v {
        push(writer, rl, MIN_WRITE_INTERVAL, "venus_shunt_voltage_v",
             || Sample::new("venus_shunt_voltage_v", ts, v as f64));
    }
    if let Some(v) = shunt.current_a {
        push(writer, rl, MIN_WRITE_INTERVAL, "venus_shunt_current_a",
    ...
    if let Some(v) = shunt.time_to_go_min {
        push(writer, rl, MIN_WRITE_INTERVAL, "venus_shunt_time_to_go_min",
    ...
    if let Some(code) = shunt.state.as_ref().map(|s| shunt_state_to_code(s)) {
        push(writer, rl, MIN_WRITE_INTERVAL, "venus_shunt_state",
...
pub fn write_venus_inverter(writer: &Writer, rl: &RateLimiter, inv: &VenusInverter) {
    ...
    if let Some(v) = inv.voltage_v {
        push(writer, rl, MIN_WRITE_INTERVAL, "venus_inverter_voltage_v",
             || Sample::new("venus_inverter_voltage_v", ts, v as f64));
    }
    if let Some(v) = inv.current_a {
        push(writer, rl, MIN_WRITE_INTERVAL, "venus_inverter_current_a",
    ...
    push(writer, rl, MIN_WRITE_INTERVAL, "venus_inverter_state",
         || Sample::new("venus_inverter_state", ts, st_code as f64));
    let mode_code = inverter_mode_to_code(&inv.mode);
    push(writer, rl, MIN_WRITE_INTERVAL, "venus_inverter_mode",
...
pub fn write_ats(writer: &Writer, rl: &RateLimiter, snap: &AtsSnapshot) {
    ...
    push(writer, rl, MIN_WRITE_INTERVAL, "ats_active_source",
         || Sample::new("ats_active_source", ts, snap.active_source as i32 as f64));

    push(writer, rl, MIN_WRITE_INTERVAL, "ats_v1a",
         || Sample::new("ats_v1a", ts, snap.v1a as f64));
    push(writer, rl, MIN_WRITE_INTERVAL, "ats_v1b",
         || Sample::new("ats_v1b", ts, snap.v1b as f64));
    push(writer, rl, MIN_WRITE_INTERVAL, "ats_v1c",
         || Sample::new("ats_v1c", ts, snap.v1c as f64));
    push(writer, rl, MIN_WRITE_INTERVAL, "ats_v2a",
         || Sample::new("ats_v2a", ts, snap.v2a as f64));
    push(writer, rl, MIN_WRITE_INTERVAL, "ats_v2b",
         || Sample::new("ats_v2b", ts, snap.v2b as f64));
    push(writer, rl, MIN_WRITE_INTERVAL, "ats_v2c",
         || Sample::new("ats_v2c", ts, snap.v2c as f64));

    push(writer, rl, MIN_WRITE_INTERVAL, "ats_freq1_hz",
         || Sample::new("ats_freq1_hz", ts, snap.freq1_hz.unwrap_or(0) as f64));
    push(writer, rl, MIN_WRITE_INTERVAL, "ats_freq2_hz",
         || Sample::new("ats_freq2_hz", ts, snap.freq2_hz.unwrap_or(0) as f64));

    push(writer, rl, MIN_WRITE_INTERVAL, "ats_sw1_closed",
         || Sample::new("ats_sw1_closed", ts, snap.sw1_closed as i32 as f64));
    push(writer, rl, MIN_WRITE_INTERVAL, "ats_sw2_closed",
    ...
    push(writer, rl, MIN_WRITE_INTERVAL, "ats_voltage_v",
         || Sample::new("ats_voltage_v", ts, ats_voltage_v));
    push(writer, rl, MIN_WRITE_INTERVAL, "ats_freq_hz",
         || Sample::new("ats_freq_hz", ts, af_hz as f64));

    // ── Code de défaut (0 = aucun, 1..7 = défauts variés) ───────────────────
    push(writer, rl, MIN_WRITE_INTERVAL, "ats_fault",
         || Sample::new("ats_fault", ts, snap.fault as i32 as f64));

    // ── Modes / flags ATS ───────────────────────────────────────────────────
    push(writer, rl, MIN_WRITE_INTERVAL, "ats_sw_mode",
         || Sample::new("ats_sw_mode", ts, snap.sw_mode as i32 as f64));
    push(writer, rl, MIN_WRITE_INTERVAL, "ats_remote",
         || Sample::new("ats_remote", ts, snap.remote as i32 as f64));
    push(writer, rl, MIN_WRITE_INTERVAL, "ats_middle_off",
    ...
    for (name, status) in phases {
        push(writer, rl, MIN_WRITE_INTERVAL, name,
             || Sample::new(name, ts, status as i32 as f64));
    }

    // ── Compteurs de commutation et runtime ─────────────────────────────────
    push(writer, rl, ENERGY_WRITE_INTERVAL, "ats_cnt1",
         || Sample::new("ats_cnt1", ts, snap.cnt1 as f64));
    push(writer, rl, ENERGY_WRITE_INTERVAL, "ats_cnt2",
         || Sample::new("ats_cnt2", ts, snap.cnt2 as f64));
    push(writer, rl, ENERGY_WRITE_INTERVAL, "ats_runtime_h",
         || Sample::new("ats_runtime_h", ts, snap.runtime_h as f64));

    // ── Tensions max historiques ────────────────────────────────────────────
    push(writer, rl, ENERGY_WRITE_INTERVAL, "ats_max1_v",
         || Sample::new("ats_max1_v", ts, snap.max1_v as f64));
    push(writer, rl, ENERGY_WRITE_INTERVAL, "ats_max2_v",
...
pub fn write_shelly(writer: &Writer, rl: &RateLimiter, snap: &ShellyEmSnapshot) {
    ...
    push(writer, rl, MIN_WRITE_INTERVAL, &format!("shelly_power_w:{}", id),
         || Sample::new("shelly_power_w", ts, snap.total_power_w as f64).with_label("id", id.clone()));

    push(writer, rl, MIN_WRITE_INTERVAL, &format!("shelly_voltage_v:{}", id),
    ...
    for (ch_idx, ch) in [(0u8, &snap.channel_0), (1u8, &snap.channel_1)] {
        ...
        push(writer, rl, MIN_WRITE_INTERVAL, &format!("shelly_channel_power_w:{}:{}", id, ch_idx),
             || Sample::new("shelly_channel_power_w", ts, ch.power_w as f64)
                .with_label("id", id.clone()).with_label("channel", ch_label.clone()));
        push(writer, rl, MIN_WRITE_INTERVAL, &format!("shelly_current_a:{}:{}", id, ch_idx),
             || Sample::new("shelly_current_a", ts, ch.current_a as f64)
                .with_label("id", id.clone()).with_label("channel", ch_label.clone()));
        push(writer, rl, MIN_WRITE_INTERVAL, &format!("shelly_output:{}:{}", id, ch_idx),
             || Sample::new("shelly_output", ts, ch.output as i32 as f64)
                .with_label("id", id.clone()).with_label("channel", ch_label.clone()));
        push(writer, rl, ENERGY_WRITE_INTERVAL, &format!("shelly_energy_wh:{}:{}", id, ch_idx),
             || Sample::new("shelly_energy_wh", ts, ch.energy_wh)
                .with_label("id", id.clone()).with_label("channel", ch_label.clone()));
        push(writer, rl, MIN_WRITE_INTERVAL, &format!("shelly_power_factor:{}:{}", id, ch_idx),
             || Sample::new("shelly_power_factor", ts, ch.power_factor as f64)
                .with_label("id", id.clone()).with_label("channel", ch_label.clone()));
        push(writer, rl, ENERGY_WRITE_INTERVAL, &format!("shelly_returned_wh:{}:{}", id, ch_idx),
             || Sample::new("shelly_returned_wh", ts, ch.returned_wh)
                .with_label("id", id.clone()).with_label("channel", ch_label));
    }
    if let Some(rssi) = snap.rssi {
        push(writer, rl, TEMP_WRITE_INTERVAL, &format!("shelly_rssi:{}", snap.id),
...
pub fn write_solar_total(writer: &Writer, rl: &RateLimiter, solar_total_w: f32) {
    let ts = now_ms();
    push(writer, rl, MIN_WRITE_INTERVAL, "solar_total_w",
         || Sample::new("solar_total_w", ts, solar_total_w as f64));
    // Alias attendu par les dashboards Grafana drilldown.
    push(writer, rl, MIN_WRITE_INTERVAL, "total_solar_power",
...
pub fn write_solar_components(writer: &Writer, rl: &RateLimiter, dc_pv_w: Option<f32>, pvinv_w: Option<f32>) {
    ...
    if let Some(v) = dc_pv_w {
        push(writer, rl, MIN_WRITE_INTERVAL, "dc_pv_power_w",
             || Sample::new("dc_pv_power_w", ts, v as f64));
    }
    if let Some(v) = pvinv_w {
        push(writer, rl, MIN_WRITE_INTERVAL, "pvinv_power_w",
...
pub fn write_solar_yield(writer: &Writer, rl: &RateLimiter, yield_kwh: f32) {
    let ts = now_ms();
    push(writer, rl, ENERGY_WRITE_INTERVAL, "solar_yield_kwh",
         || Sample::new("solar_yield_kwh", ts, yield_kwh as f64));
    push(writer, rl, ENERGY_WRITE_INTERVAL, "solar_total_wh",
...
pub fn write_monitor(writer: &Writer, rl: &RateLimiter, snap: &crate::state::MonitorSnapshot) {
    ...
    for proc in &snap.processes {
        ...
        push(writer, rl, MIN_WRITE_INTERVAL, &format!("pi5_process_cpu_percent:{}", proc.name),
             || Sample::new("pi5_process_cpu_percent", ts, proc.cpu_percent as f64)
                .with_label("process", proc.name.clone()));
        push(writer, rl, MIN_WRITE_INTERVAL, &format!("pi5_process_mem_mb:{}", proc.name),
...
pub fn write_wh_metrics(writer: &Writer, rl: &RateLimiter, m: &WhMetricsPayload) {
    ...
    if let Some(v) = m.current_temp_c {
        push(writer, rl, TEMP_WRITE_INTERVAL, "wh_current_temp_c",
             || Sample::new("wh_current_temp_c", ts, v as f64));
    }
    if let Some(v) = m.target_temp_c {
        push(writer, rl, TEMP_WRITE_INTERVAL, "wh_target_temp_c",
             || Sample::new("wh_target_temp_c", ts, v as f64));
    }
    if let Some(v) = m.mode {
        push(writer, rl, MIN_WRITE_INTERVAL, "wh_mode",
